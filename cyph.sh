...
if [ $? -eq 0 ]; then
  ...
  cd /Users/adeponte/Documents/programming/sandbox/cyphactor.github.com
  ...
  tmux new-window -t "${TMUX_SESSION_NAME}" -n jekyll -d '/bin/zsh'
  tmux send-keys -t "${TMUX_SESSION_NAME}:1" 'jekyll --auto --server' C-m
